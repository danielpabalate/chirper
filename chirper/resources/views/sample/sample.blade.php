    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            @foreach ($samples as $sample)
                ...
                @php
                    $calculatedResult = 0;
                    $number1 = $sample->number1;
                    $number2 = $sample->number2;
                    $solution = $sample->solution;

                    if ($sample->solution != null) {
                        switch ($solution) {
                            case '+':
                            ...
                            default:
                                $calculatedResult = "Error! Invalid solution.";
                                break;
                        }
                    } else {
                        $calculatedResult = "Error! Please check solution.";
                    }
                @endphp
                ...
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="text-gray-800">{{ $sample->user->name }}</span>
                                <small class="ml-2 text-sm text-gray-600">{{ $sample->created_at->format('j M Y, g:i a') }}</small>
                                @unless ($sample->created_at->eq($sample->updated_at))
                                    <small class="text-sm text-gray-600"> &middot; {{ __('edited') }}</small>
                                @endunless
                            </div>

                            @if ($sample->user->is(auth()->user()))
                                <x-dropdown>
                                    <x-slot name="trigger">
                                        <button>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('sample.edit', $sample)">
                                            {{ __('Edit') }}
                                        </x-dropdown-link>
                                        <form method="POST" action="{{ route('sample.destroy', $sample) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('sample.destroy', $sample)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
                        ...
                        <p class="mt-4 text-lg text-gray-900">
                            {{ $sample->number1 }} {{ $sample->solution }} {{ $sample->number2 }} = {{$calculatedResult}}
                        </p>
                    </div>
                </div>
            @endforeach
